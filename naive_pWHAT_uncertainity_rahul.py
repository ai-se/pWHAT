...
def find_variance_ranks(rest_indep):
    ...
    variances_ranks = [i[0] for i in sorted(enumerate(variance_scores), key=lambda x:x[1])]
...
def run_experiment(dataset_name):
    ...
    temp_filename = "./temp_where_2.csv"
    ...
    assert (
    sum([len(cluster) for cluster in clusters]) == training_set_size), "Something is wrong with the clustering method"
    ...
    assert(sum([len(cluster) for cluster in rest_dep]) + len(training_dep) == training_set_size), "train and dep should be equal to training set size"
