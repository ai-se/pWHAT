...
def run_experiment_3(dataset_name, count_member=None):
    ...
    shuffle(indexes)
    # getting the reserve testing data
    training_indexes = indexes[:(int(len(raw_data)*0.4))]
    training_data = [data[i] for i in training_indexes]

    # getting the rest of the data
    from copy import deepcopy
    testing_data = deepcopy(data)
    for i in sorted(training_indexes, reverse=True): testing_data.pop(i)

    assert(len(training_data) + len(testing_reserve) + len(testing_data) == len(raw_data)), "something is wrong"

    # adding hotspot scores to all the data points
    training_data = get_hotspot_scores(training_data)
    ...
    fault_rate = 1
    count_member = 0
    while fault_rate > .07 and count_member < 0.4 * len(data):
        training_indices = find_hotspot(training_data, count_member+1)
    ...
    indep_testing_set = [td.decisions for td in testing_data]
    dep_testing_set = [td.objective for td in testing_data]
...
def run_experiment_where(dataset_name, count_member=None):
    def transform(filename):
        return "./Data/input/" + filename
    # read raw data
    filename = transform(dataset_name)
    clusters = WHEREDataTransformation(filename)
    new_clusters = []
...
def experiment_hotspot_spectral():
    ...
    datasets = [['apache', 16], ['bj', 16], ['llvm',32], ['x264', 32], ['sqlite', 64], ['bc', 64],]
    for dataset in datasets:
        ...
        for _ in xrange(10):
            ...
            import sys
            sys.stdout.flush()
            mre, count = run_experiment_where(dataset[0], dataset[1])
            scores.append(mre)
            counts.append(count)
...
def experiment_what_hotspot_spectral():
    ...
    for dataset in datasets:
        ...
        for count in xrange(1, 5):
            ...
            for _ in xrange(10):
                print ". ",
                sys.stdout.flush()
                data, training_set = run_experiment_where(dataset, count)

                training_set_indices = [ts.id for ts in training_set]
                data_indices = [d.id for d in data]
                for tsi in training_set_indices: data_indices.remove(tsi)

                data_dict = {}
                for d in data: data_dict[d.id] = d
                ...
                mean_list.append(mean(mre))
            print len(training_set), "Mean: ", mean(mean_list), std(mean_list)
...
if __name__ == "__main__":
    import sys
    # experiment_what_hotspot_spectral()
    ...
    for dataset in datasets:
        ...
        for _ in xrange(10):
            print ".",
            sys.stdout.flush()
            mre, datalength = run_experiment_3(dataset)
            mean_mre.append(mre)
            mean_length.append(datalength)
        from numpy import mean, std
        print
        print dataset, mean(mean_mre), mean(mean_length), std(mean_mre), std(mean_length)
        print [round(r, 3)*100 for r in mean_mre]
